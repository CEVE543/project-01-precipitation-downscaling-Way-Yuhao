...
def cvt_to_tensor(*args):
    tensors = []
    for arr in args:
        t = torch.from_numpy(arr.to_numpy()).unsqueeze(dim=0).unsqueeze(dim=0)
        tensors += t
...
def main(config):
    ...
    # PC super-resolution
    config.data.resolution_ratio = 16
    ...
    cpc_path = '/home/yl241/data/CLIMATE/CPC_precipitation/precip_tx.nc'
    houston_da = xr.open_dataarray(cpc_path)
    # harvey
    # d1 = houston_da.sel(time='2017-08-25')
    # d2 = houston_da.sel(time='2017-08-26')
    # d3 = houston_da.sel(time='2017-08-27')
    # d4 = houston_da.sel(time='2017-08-28')

    d1 = houston_da.sel(time='2023-09-25')
    d2 = houston_da.sel(time='2023-09-26')
    d3 = houston_da.sel(time='2023-09-27')
    d4 = houston_da.sel(time='2023-09-28')

    img = cvt_to_tensor(d1, d2, d3, d4)
    ...
    # normalize
    valid_region = img != -1
    scaled = torch.zeros_like(img)
    scaled[valid_region] = torch.log(img[valid_region] + 0.00001)
    # normalized = scaled
    normalized = scaled / 5.  # scale to 0 to 1
    normalized[normalized < 0.17] = 0  # clip lower end at theta
    low_res = normalized

    low_res = low_res.to(config.device)
    low_res_display = F.interpolate(low_res, size=(256, 256), mode='nearest')
    baseline = F.interpolate(low_res, size=(256, 256), mode='bilinear')
    valid_region = F.interpolate(valid_region, size=(256, 256), mode='bilinear')
    # baseline[valid_region] = -1
    ...
    show_samples(low_res_display, size=16 * 24, config=config, caption='input',
                 save_path=os.path.join(save_plt_path, 'input.png'))
    show_samples(baseline, size=16 * 24, config=config, caption='baseline',
                 save_path=os.path.join(save_plt_path, 'baseline.png'))
