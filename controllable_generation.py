...
def get_pc_cfg_upsampler(sde, predictor, corrector, inverse_scaler, snr,
    ...
    def pc_upsampler(model, condition, w, out_dim, save_dir=None, null_token=None):
        with torch.no_grad():
            # Initial sample
            x = sde.prior_sampling(out_dim).to(condition.device)
            timesteps = torch.linspace(sde.T, eps, sde.N)
            # class conditional
            # null_condition = torch.ones(condition.shape[0], device=condition.device) * null_token
            # null_condition = null_condition.int()
            # super resolution
            null_condition = torch.ones_like(x) * null_token
            for i in track(range(sde.N), description=f'Sampling {sde.N} steps....', refresh_per_second=1):
                t = timesteps[i]
                # forward pass through conditional model
                # x_c, x_mean_c = corrector_upsample_update_fn(model, x=x, t=t, low_res_img=low_res_img)
                # x_c, x_mean_c = projector_upsample_update_fn(model, x=x_c, t=t, low_res_img=low_res_img)
                ...
                if i in show_vis_at_steps:  # show diffusion progression
                    ...
                    plt.title(f'progress, i = {i}, mean = {x[1, :, :, :].mean():.2f}')
                    plt.colorbar()
                    # plt.show()
                    plt.savefig(f'{save_dir}/progression_{i}.png')
                    plt.close()
